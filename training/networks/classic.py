    ...
    def __init__(self,
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                cout = model_channels
                cy = label_dim
                self.enc[f'{res}x{res}_conv'] = Conv2d(in_channels=cin+cy, out_channels=cout, kernel=3, **init)
            else:
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
        ...
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
        label_dim           = 0,            # Nunber of color channels at label.
        augment_dim         = 0,            # Augmentation label dimensionality, 0 = no augmentation.
        ...
        self.map_layer1 = Linear(in_features=emb_channels, out_features=emb_channels, **init)
        self.map_label = nn.Identity()

        # Chris B: in this class `label_dim` is now the channel dim of the
        # concatenated y, not some scalar class variable.
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
        label_dim       = 0,                # Number of class color channels.
        use_fp16        = False,            # Execute the underlying model at FP16 precision?
