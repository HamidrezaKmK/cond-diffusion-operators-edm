...
def deterministic_edm_sampler(
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        # print("---")
        denoised = net(coords=coords, samples=x_hat, sigma=t_hat.repeat(x_hat.shape[0]), conditioning=class_labels).to(torch.float32)
        d_cur = (x_hat - denoised) / t_hat
        x_next = x_hat + (t_next - t_hat) * d_cur

        # Apply 2nd order correction.
        if i < num_steps - 1:
            denoised = net(coords=coords, samples=x_next, sigma=t_hat.repeat(x_hat.shape[0]), conditioning=class_labels).to(torch.float32)
